...
import CrossModal from "../appcomponents/CrossModal";
...
const Settings = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.put(
        `${endpoint}/activity/addProfileImage`,
        formData,
        {
          headers: {
  ...
  React.useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const response = await axios.get<ProfileResponseDTO>(
    ...
    <div className="w-full flex flex-row relative">
          ...
          <div>
            ...
            {chnageName && thingtoChange === "name" && (
              <div className="flex flex-row mt-3 gap-1">
                ...
                />
                <Button className="h-[40px] bg-black text-white cursor-pointer">
                  Add
                </Button>
              </div>
            )}
          ...
          <div>
            {!profile.userSeller && (
              <Button
                ...
                className="py-3 cursor-pointer min-h-[40px]"
              >
                Become a Seller
              </Button>
            )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center ">
          <TickModal text="Success" />
        </div>
      )}
      {showCross && (
        <div className="fixed inset-0 z-50 flex items-center justify-center ">
          <CrossModal text="Failed Bhaiya" />
        </div>
      )}
    </div>
  );
